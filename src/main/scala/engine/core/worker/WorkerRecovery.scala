...
trait WorkerRecovery extends ControlRecovery {
  ...
  private lazy val inputSerializer = new Output(
    new FileOutputStream("D:\\" + amberID.toString + ".in", true)
  )
  private lazy val outputSerializer = new PrintWriter(
    new FileOutputStream("D:\\" + amberID.toString + ".out", true)
  )
  ...
  override def saveOutputControlMessageID(id: Long): Unit = {
    ...
    } finally outputSerializer.flush()
  }
